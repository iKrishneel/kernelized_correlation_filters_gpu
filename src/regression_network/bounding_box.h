...
public:
  BoundingBox2D();
  BoundingBox2D(const std::vector<float>& bounding_box);
  ...
  void Scale(const cv::Mat& image, BoundingBox2D* bbox_scaled) const;

  // Unnormalize the size of the bounding box based on the size of the image.
  // (Undoes the effect of Scale).
  void Unscale(const cv::Mat& image, BoundingBox2D* bbox_unscaled) const;
  ...
  void Recenter(const BoundingBox2D& search_location,
                const double edge_spacing_x, const double edge_spacing_y,
                BoundingBox2D* bbox_recentered) const;

  // Undo the effect of Recenter.
  void Uncenter(const cv::Mat& raw_image, const BoundingBox2D& search_location,
                const double edge_spacing_x, const double edge_spacing_y,
  ...
  double compute_intersection(const BoundingBox2D& bbox) const;
