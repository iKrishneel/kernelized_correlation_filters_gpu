...
BoundingBox2D::BoundingBox2D(const std::vector<float>& bounding_box)
  : scale_factor_(kScaleFactor)
{
...
void BoundingBox2D::GetVector(std::vector<float>* bounding_box) const {
  if (use_coordinates_output) {
...
void BoundingBox2D::Print() const {
  printf("Bounding box: x,y: %lf, %lf, %lf, %lf, w,h: %lf, %lf\n", x1_, y1_, x2_, y2_, get_width(), get_height());
}

void BoundingBox2D::Scale(const cv::Mat& image, BoundingBox2D* bbox_scaled) const {
  *bbox_scaled = *this;
...
void BoundingBox2D::Unscale(const cv::Mat& image, BoundingBox2D* bbox_unscaled) const {
  *bbox_unscaled = *this;
...
double BoundingBox2D::compute_output_width() const {
...
double BoundingBox2D::compute_output_height() const {
...
double BoundingBox2D::get_center_x() const {
  // Compute the bounding box center x-coordinate.
  return (x1_ + x2_) / 2;
}

double BoundingBox2D::get_center_y() const {
  // Compute the bounding box center y-coordinate.
  return (y1_ + y2_) / 2;
}

void BoundingBox2D::Recenter(const BoundingBox2D& search_location,
              const double edge_spacing_x, const double edge_spacing_y,
              BoundingBox2D* bbox_gt_recentered) const {
...
void BoundingBox2D::Uncenter(const cv::Mat& raw_image,
                           const BoundingBox2D& search_location,
                           const double edge_spacing_x, const double edge_spacing_y,
                           BoundingBox2D* bbox_uncentered) const {
...
double BoundingBox2D::edge_spacing_x() const {
...
double BoundingBox2D::edge_spacing_y() const {
...
void BoundingBox2D::Draw(const int r, const int g, const int b,
                       cv::Mat* image) const {
...
void BoundingBox2D::DrawBoundingBox(cv::Mat* image) const {
  // Draw a white bounding box on the image.
  Draw(255, 255, 255, image);
}

void BoundingBox2D::Shift(const cv::Mat& image,
...
double BoundingBox2D::compute_intersection(const BoundingBox2D& bbox) const {
...
double BoundingBox2D::compute_area() const {
